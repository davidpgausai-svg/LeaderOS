...
import { insertStrategySchema, insertProjectSchema, insertActionSchema, insertActionDocumentSchema, insertActionChecklistItemSchema, insertBarrierSchema, insertDependencySchema, insertTemplateTypeSchema, insertExecutiveGoalSchema, insertTeamTagSchema, insertUserStrategyAssignmentSchema, insertProjectResourceAssignmentSchema, insertActionPeopleAssignmentSchema, insertPtoEntrySchema, insertHolidaySchema, insertIntakeFormSchema, insertDecisionSchema, insertDecisionRaciSchema, insertWorkstreamSchema, insertPhaseSchema, insertWorkstreamDependencySchema, insertGateCriteriaSchema } from "@shared/schema";
...
Respond ONLY with a valid JSON object in this exact format:
{
  ...
  // List all intake forms for the organization (admin and co-lead for read access)
  app.get("/api/intake-forms", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await storage.getUser(userId);
      if (!user || !user.organizationId) {
      ...
      const forms = await storage.getIntakeFormsByOrganization(user.organizationId);
      res.json(forms);
    } catch (error) {
      logger.error("Failed to fetch intake forms", error);
      res.status(500).json({ message: "Failed to fetch intake forms" });
    }
  });

  // Get a single intake form (admin only)
  app.get("/api/intake-forms/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await storage.getUser(userId);
      if (!user || !user.organizationId) {
      ...
      logger.error("Failed to fetch intake form", error);
      res.status(500).json({ message: "Failed to fetch intake form" });
    }
  });

  // Create intake form (admin only)
  app.post("/api/intake-forms", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await storage.getUser(userId);
      if (!user || !user.organizationId) {
      ...
      const validatedData = insertIntakeFormSchema.parse(req.body);
      ...
      logger.error("Failed to create intake form", error);
      res.status(500).json({ message: "Failed to create intake form" });
    }
  });

  // Update intake form (admin only)
  app.patch("/api/intake-forms/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await storage.getUser(userId);
      if (!user || !user.organizationId) {
      ...
      const existing = await storage.getIntakeForm(req.params.id);
      if (!existing || existing.organizationId !== user.organizationId) {
        return res.status(404).json({ message: "Form not found" });
      }
      const validatedData = insertIntakeFormSchema.partial().parse(req.body);
      const form = await storage.updateIntakeForm(req.params.id, validatedData);
      res.json(form);
    } catch (error) {
      if (error instanceof ZodError) {
        return res.status(400).json({ message: "Invalid input", errors: error.errors });
      }
      logger.error("Failed to update intake form", error);
      res.status(500).json({ message: "Failed to update intake form" });
    }
  });

  // Delete intake form (admin only)
  app.delete("/api/intake-forms/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await storage.getUser(userId);
      if (!user || !user.organizationId) {
      ...
      logger.error("Failed to delete intake form", error);
  ...
  // Get public form data by slug (no auth required)
  app.get("/api/public/intake/:slug", async (req, res) => {
    try {
      const form = await storage.getIntakeFormBySlug(req.params.slug);
      if (!form) {
        return res.status(404).json({ message: "Form not found" });
      }
      ...
      if (form.maxTotalSubmissions) {
        const totalCount = await storage.countTotalSubmissions(form.id);
        if (totalCount >= form.maxTotalSubmissions) {
          return res.status(410).json({ message: "This form has reached its maximum number of submissions" });
        }
      }
      res.json({
        id: form.id,
      ...
      logger.error("Failed to fetch public intake form", error);
      res.status(500).json({ message: "Failed to load form" });
    }
  });

  // Submit to public form (no auth required)
  app.post("/api/public/intake/:slug", async (req, res) => {
    try {
      const form = await storage.getIntakeFormBySlug(req.params.slug);
      if (!form) {
        return res.status(404).json({ message: "Form not found" });
      ...
      if (form.maxSubmissionsPerEmail && emailLower) {
        const emailCount = await storage.countSubmissionsByEmail(form.id, emailLower);
        if (emailCount >= form.maxSubmissionsPerEmail) {
          return res.status(429).json({ message: "You have reached the maximum number of submissions for this form" });
        }
      }
      ...
      res.status(201).json({
        success: true,
        thankYouMessage: form.thankYouMessage || "Thank you for your submission!",
      });
    } catch (error) {
  ...
  // ==================== INTAKE SUBMISSIONS (authenticated) ====================

  // List all submissions for the organization
  app.get("/api/intake-submissions", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await storage.getUser(userId);
      if (!user || !user.organizationId) {
      ...
      const submissions = await storage.getIntakeSubmissionsByOrganization(user.organizationId);
      res.json(submissions);
    } catch (error) {
      logger.error("Failed to fetch intake submissions", error);
      res.status(500).json({ message: "Failed to fetch submissions" });
    }
  });

  // Get submissions for a specific form
  app.get("/api/intake-forms/:formId/submissions", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await storage.getUser(userId);
      if (!user || !user.organizationId) {
      ...
      const form = await storage.getIntakeForm(req.params.formId);
      if (!form || form.organizationId !== user.organizationId) {
        return res.status(404).json({ message: "Form not found" });
      }
      const submissions = await storage.getIntakeSubmissionsByForm(req.params.formId);
      res.json(submissions);
    } catch (error) {
  ...
  app.get("/api/intake-submissions/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await storage.getUser(userId);
      if (!user || !user.organizationId) {
      ...
      if (!submission || submission.organizationId !== user.organizationId) {
        return res.status(404).json({ message: "Submission not found" });
      }
      res.json(submission);
    } catch (error) {
      logger.error("Failed to fetch intake submission", error);
      res.status(500).json({ message: "Failed to fetch submission" });
    }
  });

  // Update submission (status, assignment)
  app.patch("/api/intake-submissions/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await storage.getUser(userId);
      if (!user || !user.organizationId) {
      ...
      const existing = await storage.getIntakeSubmission(req.params.id);
      if (!existing || existing.organizationId !== user.organizationId) {
        return res.status(404).json({ message: "Submission not found" });
      }
      const { status, assignedStrategyId, assignedProjectId, assignedActionId } = req.body;
      const updates: any = {};
      if (status) updates.status = status;
      if (assignedStrategyId !== undefined) updates.assignedStrategyId = assignedStrategyId;
      if (assignedProjectId !== undefined) updates.assignedProjectId = assignedProjectId;
      if (assignedActionId !== undefined) updates.assignedActionId = assignedActionId;
      if (status === 'under_review' || status === 'assigned' || status === 'dismissed') {
        updates.reviewedBy = userId;
        updates.reviewedAt = new Date();
      }
      const submission = await storage.updateIntakeSubmission(req.params.id, updates);
      res.json(submission);
    } catch (error) {
