...
import { Sparkles, Loader2, Info, X, Plus } from "lucide-react";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export function CreateStrategyModal({ open, onOpenChange }: CreateStrategyModalProps) {
  ...
  const createStrategyMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || error.error || "Failed to create priority",
        variant: "destructive",
      });
    },
  });
  ...
  const handleGenerateContinuum = async () => {
    ...
    try {
      ...
      const visibleKeys = BUILT_IN_SECTIONS.filter(s => !hiddenSections.has(s.key)).map(s => s.key);
      const hasAllVisible = visibleKeys.every(key => data[key]);
      if (!hasAllVisible) {
