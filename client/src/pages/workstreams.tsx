...
export default function Workstreams() {
  ...
  const { data: strategy } = useQuery<any>({
    ...
    select: (data: any[]) => data?.find((s: any) => s.id === strategyId),
  });
  ...
  const { data: tasks = [], isLoading: taskLoading } = useQuery<WorkstreamTask[]>({
  ...
  const getTasksForCell = (workstreamId: string, phaseId: string) => {
    return tasks.filter(t => t.workstreamId === workstreamId && t.phaseId === phaseId);
  };

  const getGateCriteriaQueryKey = (gateTaskId: string) => [`/api/gate-criteria?gateTaskId=${gateTaskId}`];
  ...
  const seedMutation = useMutation({
    mutationFn: () => apiRequest("POST", "/api/workstreams/seed-program", { strategyId }),
    onSuccess: () => {
      invalidateAll();
      toast({ title: "Success", description: "ERP Program structure created successfully" });
    },
    onError: () => {
      toast({ title: "Error", description: "Failed to seed ERP program", variant: "destructive" });
    },
  });
  ...
  const isLoading = wsLoading || phLoading || taskLoading;
          ...
          <div className="flex items-center justify-between">
            ...
            {workstreams.length === 0 && canEdit && (
              ...
              >
                ...
                {seedMutation.isPending ? "Setting up..." : "Setup ERP Program"}
              </Button>
            )}
          </div>
        </header>
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center">
          ...
          <p className="text-gray-600">Use the "Setup ERP Program" button to create the program structure.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center">
          ...
          <p className="text-gray-600">Setup the ERP Program to see executive overview.</p>
        </CardContent>
      </Card>
    );
  }
